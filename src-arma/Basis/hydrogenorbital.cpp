...
void HydrogenOrbital::setParameters(arma::vec parameters)
{
...
{
    arma::uword number = 1;
    arma::uword number2 = 0;
    arma::uword maxNumber = 0;
    arma::uword counter = 1;
    m_numberOfShells = 1;
    while (true) {
        number2 += 2 * number;
        if (m_numberOfParticles == number2) {
            ...
            std::cout << "Only closed shell atoms are accepted: He, Be, Ne, Mg, Ar, Zn, Kr"
                      << std::endl;
            MPI_Finalize();
            exit(0);
        } else {
...
{
    arma::uword sum = 0;
    for (arma::uword i = 1; i < m_numberOfShells + 1; i++) {
        sum += i * i;
    }
    m_LOS.zeros(sum, 3);
    arma::uword i = 0;
    for (arma::sword n = 1; n < signed(m_numberOfShells + 1); n++) {
        for (arma::sword l = 0; l < n; l++) {
            for (arma::sword m = -l; m < l + 1; m++) {
                m_LOS(i, 0) = n;
                m_LOS(i, 1) = l;
                m_LOS(i, 2) = m;
                i++;
...
double HydrogenOrbital::basisElement(const arma::uword n, arma::vec position)
{
    return evaluateCart(position, m_LOS(n, 0), m_LOS(n, 1), m_LOS(n, 2));
}

double HydrogenOrbital::basisElementDer(const arma::uword n, const arma::uword i, arma::vec position)
{
    return evaluateCartDerivative(position, i, m_LOS(n, 0), m_LOS(n, 1), m_LOS(n, 2));
}

double HydrogenOrbital::basisElementSecDer(const arma::uword n, const arma::uword i, arma::vec position)
{
    return evaluateCartSecondDerivative(position, i, m_LOS(n, 0), m_LOS(n, 1), m_LOS(n, 2));
}

double HydrogenOrbital::basisElementPar(const arma::uword n, arma::vec position)
{
...
double HydrogenOrbital::evaluateCart(arma::vec position, arma::uword n, arma::uword l, arma::sword m)
{
...
double HydrogenOrbital::evaluateCartDerivative(arma::vec position, arma::uword i, arma::uword n, arma::uword l, arma::sword m)
{
        ...
        if (l == 0) {
            ...
            if (m == 0) {
                if (i == 0) {
                    ...
                    result = -k * t * z * (k * r - 20) * (k * r - 8) + 320 * r - 80 * k * r * r
                             + 4 * k * k * r * r * r;
                }
            } else if (m == 1) {
                if (i == 0) {
                    result = -k * t * x * (k * r - 20) * (k * r - 8) + 320 * r - 80 * k * r * r
                             + 4 * k * k * r * r * r;
                } else if (i == 1) {
                ...
                if (i == 0) {
                    ...
                    result = -k * t * y * (k * r - 20) * (k * r - 8) + 320 * r - 80 * k * r * r
                             + 4 * k * k * r * r * r;
                } else if (i == 2) {
                    result = -k * t * y * (k * r - 20) * (k * r - 8);
                }
...
double HydrogenOrbital::evaluateCartSecondDerivative(
    arma::vec position, arma::uword i, arma::uword n, arma::uword l, arma::sword m)
{
    ...
    if (n == 1) {
        ...
        if (l == 0) {
            result = k
                     * (r * r * (2 * k * r * (5 - 3 * k * r) - 27)
                        + t * t * (k * r * (2 * k * r * (k * r - 18) + 9) + 27))
                     / 9;
        } else if (l == 1) {
            if (m == 0) {
        ...
        if (l == 0) {
            result = k * (k * r - 32)
                     * (k * k * k * r * r * r - 24 * k * k * r * r + 144 * k * r - 192) / (16 * r);
        } else if (l == 1) {
            if (m == 0) {
...
double HydrogenOrbital::evaluate(double x, arma::uword n)
{
    return x + n;
}

double HydrogenOrbital::evaluateDerivative(double x, arma::uword n)
{
    return x + n;
}

double HydrogenOrbital::evaluateSecondDerivative(double x, arma::uword n)
{
    return x + n;
}
