...
Sampler::Sampler(System *system)
{
    ...
    m_numberOfStepsPerBatch = arma::uword(m_stepsWOEqui / m_numberOfBatches);
    ...
    m_densityGrid.zeros(m_numberOfBins, m_numberOfBins);
    m_particlesPerBin.zeros(m_numberOfBins);
    m_particlesPerBinPairwise.zeros(m_numberOfBins, m_numberOfBins);
}

void Sampler::sample(const bool acceptedStep, const arma::uword stepNumber)
{
    if (stepNumber == m_equilibriationSteps) {
        ...
        m_cumulativeGradients.zeros(arma::uword(m_numberOfElements),
                                                      m_maxParameters);
        m_cumulativeGradientsE.zeros(arma::uword(m_numberOfElements),
                                                       m_maxParameters);
    }
...
{
    arma::uword parameterSlots = arma::uword(m_numberOfElements * m_maxParameters);
    m_totalCumulativeGradients.zeros(m_numberOfElements, m_maxParameters);
    m_totalCumulativeGradientsE.zeros(m_numberOfElements, m_maxParameters);
    ...
    MPI_Reduce(&m_cumulativeEnergySqrd,
               ...
               0,
               MPI_COMM_WORLD);
    MPI_Reduce(m_cumulativeGradients.memptr(),
               m_totalCumulativeGradients.memptr(),
               parameterSlots,
               MPI_DOUBLE,
               MPI_SUM,
               0,
               MPI_COMM_WORLD);
    MPI_Reduce(m_cumulativeGradientsE.memptr(),
               m_totalCumulativeGradientsE.memptr(),
               parameterSlots,
               MPI_DOUBLE,
               MPI_SUM,
               0,
...
void Sampler::setNumberOfSteps(arma::uword numberOfStepsWOEqui,
                               ...
                               arma::uword totalNumberOfStepsWEqui)
{
    ...
    m_numberOfStepsPerBatch = arma::uword(m_totalStepsWOEqui / m_numberOfBatches);
}
...
void Sampler::printOutputToTerminal(const arma::uword maxIter, const double time)
{
    m_iter += 1;
    cout << endl;
    cout << "  -- System info: "
...
void Sampler::appendInstantFiles(const std::string extension)
{
    ...
    for (arma::uword i = 1; i < m_numberOfProcesses; i++) {
...
void Sampler::openOutputFiles()
{
    ...
    if (m_printEnergyToFile && m_rank == 0) {
        ...
        //std::string averageInteractionEnergyFileName = generateFileName("energy", "_arma::uword.dat");
    ...
    if (m_printInstantEnergyToFile) {
        ...
        m_instantInteractionEnergyFileName = m_path + std::to_string(m_instantNumber) + "_"
                                             + std::to_string(m_rank) + "_arma::uword.dat";
...
void Sampler::printOneBodyDensityToFile()
{
    if (m_computeOneBodyDensity) {
        m_totalParticlesPerBin.zeros(m_numberOfBins);
        MPI_Reduce(m_particlesPerBin.memptr(),
                   m_totalParticlesPerBin.memptr(),
                   int(m_numberOfBins),
...
void Sampler::printOneBodyDensity2ToFile()
{
    if (m_computeOneBodyDensity2) {
        m_totalDensityGrid.zeros(m_numberOfBins, m_numberOfBins);
        MPI_Reduce(m_densityGrid.memptr(),
                   m_totalDensityGrid.memptr(),
                   int(m_numberOfBins * m_numberOfBins),
                   MPI_UNSIGNED_LONG_LONG,
                   MPI_SUM,
                   0,
                   MPI_COMM_WORLD);
...
void Sampler::printTwoBodyDensityToFile()
{
    if (m_computeTwoBodyDensity) {
        m_totalParticlesPerBinPairwise.zeros(m_numberOfBins, m_numberOfBins);
        MPI_Reduce(m_particlesPerBinPairwise.memptr(),
                   m_totalParticlesPerBinPairwise.memptr(),
                   int(m_numberOfBins * m_numberOfBins),
                   MPI_UNSIGNED_LONG_LONG,
                   MPI_SUM,
                   0,
...
void Sampler::computeOneBodyDensity(const arma::vec radialVector)
{
    if (m_computeOneBodyDensity) {
        for (arma::uword i_p = 0; i_p < m_numberOfParticles; i_p++) {
            if(radialVector(i_p) < m_maxRadius) {
                arma::uword bin = arma::uword(radialVector(i_p) / m_radialStep);
...
void Sampler::computeOneBodyDensity2(const arma::vec positions)
{
    if (m_computeOneBodyDensity2) {
        for (arma::uword i_p = 0; i_p < m_numberOfParticles; i_p++) {
            arma::uvec indices(m_numberOfDimensions, arma::fill::zeros);
            for (arma::uword i_d = 0; i_d < m_numberOfDimensions; i_d++) {
                arma::uword i = i_p * m_numberOfDimensions + i_d;
                indices(i_d) = arma::uword(positions(i) / (2 * m_radialStep));
            }
            if (indices(0) < m_numberOfBinsHalf && indices(1) < m_numberOfBinsHalf) {
...
void Sampler::computeTwoBodyDensity(const arma::vec radialVector)
{
    if (m_computeTwoBodyDensity) {
        for (arma::uword i_p = 0; i_p < m_numberOfParticles; i_p++) {
            arma::uword bin_i = arma::uword(radialVector(i_p) / m_radialStep) + 1;
            for (arma::uword j_p = i_p + 1; j_p < m_numberOfParticles; j_p++) {
                arma::uword bin_j = arma::uword(radialVector(j_p) / m_radialStep) + 1;
