#include "nqsjastrow3.h"
#include <cassert>
#include "wavefunction.h"
#include "../system.h"
#include <iostream>

NQSJastrow3::NQSJastrow3(System* system) :
        WaveFunction(system) {
...
double NQSJastrow3::calculateDistanceMatrixElement(const int i, const int j) {
    double dist = 0;
...
void NQSJastrow3::calculateDistanceMatrix() {
...
void NQSJastrow3::calculateDistanceMatrixCross(const int par) {
...
void NQSJastrow3::calculateG(int changedCoord) {
...
void NQSJastrow3::updateVectors() {
...
void NQSJastrow3::updateArrays(const Eigen::VectorXd positions, const int changedCoord) {
...
void NQSJastrow3::initializeArrays(const Eigen::VectorXd positions) {
    m_positions         = positions;
...
void NQSJastrow3::updateParameters(Eigen::MatrixXd parameters, const int elementNumber) {
...
double NQSJastrow3::evaluateRatio() {
    return m_probabilityRatio;
}

double NQSJastrow3::computeGradient(const int k) {
...
double NQSJastrow3::computeLaplacian() {
    double derivative = 0;
...
Eigen::VectorXd NQSJastrow3::computeParameterGradient() {
