#include "nqsjastrow2.h"
#include <cassert>
#include "wavefunction.h"
#include "../system.h"
#include <iostream>

NQSJastrow2::NQSJastrow2(System* system) :
        WaveFunction(system) {
...
void NQSJastrow2::updateVectors() {
...
void NQSJastrow2::updateArrays(const Eigen::VectorXd positions, const int changedCoord) {
    setArrays();
...
void NQSJastrow2::initializeArrays(const Eigen::VectorXd positions) {
    m_positions         = positions;
...
void NQSJastrow2::updateParameters(Eigen::MatrixXd parameters, const int elementNumber) {
...
double NQSJastrow2::evaluateRatio() {
    return m_probabilityRatio;
}

double NQSJastrow2::computeGradient(const int k) {
    double Sum = 0;
    for(int j=0; j<m_numberOfHiddenNodes; j++) {
...
double NQSJastrow2::computeLaplacian() {
    double Sum = 0;
...
Eigen::VectorXd NQSJastrow2::computeParameterGradient() {
