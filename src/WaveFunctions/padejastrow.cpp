...
double PadeJastrow::computeGradient(const int k)
{
    ...
    for (int j_p = 0; j_p < k_p; j_p++) {
        ...
        derivative -= m_f(j_p, k_p) * m_principalDistance(j, k);
    }
...
void PadeJastrow::updateMatrices(int i_p)
{
    for (int j_p = 0; j_p < i_p; j_p++) {
        double f = 1 / (1 + m_gamma * m_distanceMatrix(j_p, i_p));
        m_h(j_p, i_p) = m_distanceMatrix(j_p, i_p) * f;
        //m_h(j_p, i_p) = m_h(i_p, j_p);
        m_f(j_p, i_p) = m_beta(j_p, i_p) * f * f;
