...
void SimpleJastrow::calculateProbabilityRatio(int particle)
{
    ...
    for (int i = particle; i < m_numberOfParticles; i++) {
        ratio += m_beta(particle, i)
                 * (m_distanceMatrix(particle, i) - m_distanceMatrixOld(particle, i));
    }
...
{
    Eigen::VectorXd betaFlatten = parameters.row(m_elementNumber)
                                      .head(m_numberOfParticles * m_numberOfParticles);
...
double SimpleJastrow::computeLaplacian()
{
    ...
    for (int i = 0; i < m_degreesOfFreedom; i++) {
        ...
        for (int j_p = 0; j_p < m_numberOfParticles; j_p++) {
            ...
            if (j_p != i_p) {
                derivative += m_beta(i_p, j_p)
                              * (1
                                 - m_g(i, j) * m_g(i, j)
                                       / (m_distanceMatrix(i_p, j_p) * m_distanceMatrix(i_p, j_p)))
...
Eigen::VectorXd SimpleJastrow::computeParameterGradient()
{
    ...
    gradients.head(m_numberOfParticles * m_numberOfParticles) = WaveFunction::flatten(
        m_distanceMatrix);
    return gradients;
}
